...
<div class="container my-4">
    ...
    <?php
    $sections = [
        'Associated Contacts' => ['items' => $contacts, 'url' => "add_contact.php?client_id={$client['id']}", 'label' => '+ Add Contact', 'view' => 'contact'],
                        ...
                        <?php foreach ($data['items'] as $item): ?>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <a href="view_<?= $data['view'] ?>.php?id=<?= $item['id'] ?>">
                                    <?= htmlspecialchars($item['full_name'] ?? $item['name'] ?? $item['title']) ?>
                                </a>
