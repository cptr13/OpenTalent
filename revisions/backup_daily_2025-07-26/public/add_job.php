...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Insert job (without contact_id)
        $stmt = $pdo->prepare("INSERT INTO jobs (title, description, location, client_id, status, type) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $description, $location, $client_id, $status, $type]);
...
<div class="container my-4">
    ...
    <form method="POST">
        ...
        <div class="mb-3 position-relative">
            ...
            <input type="text" id="contact_search" class="form-control" placeholder="Start typing contact name..." autocomplete="off">
...
function setupAutocompleteWithClientFilter(inputId, resultsId, hiddenId, endpoint, clientInputId) {
    const input = document.getElementById(inputId);
    const resultsBox = document.getElementById(resultsId);
    const hiddenInput = document.getElementById(hiddenId);
    ...
    input.addEventListener('input', function () {
        ...
        const url = endpoint + '?q=' + encodeURIComponent(query) + (clientId ? '&client_id=' + encodeURIComponent(clientId) : '');
...
document.addEventListener('DOMContentLoaded', function () {
    setupAutocomplete('client_search', 'client_results', 'client_id', '/OT2/ajax/search_clients.php');
    setupAutocompleteWithClientFilter('contact_search', 'contact_results', 'contact_id', '/OT2/ajax/search_contacts.php', 'client_id');
});
</script>
