...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($job_id && $title && $location && $status) {
        try {
            // Prepare and execute update (now includes client_id and type)
            $stmt = $pdo->prepare("UPDATE jobs SET title = ?, location = ?, status = ?, client_id = ?, type = ? WHERE id = ?");
            $stmt->execute([$title, $location, $status, $client_id, $type, $job_id]);
