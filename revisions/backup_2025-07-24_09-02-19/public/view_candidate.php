...
// Load all notes for this candidate and their associations
$stmt = $pdo->prepare("
    SELECT * FROM notes 
    WHERE 
...
// Separate notes into candidate vs association
$candidate_notes = [];
$association_notes = [];
foreach ($all_notes as $note) {
    if ($note['module_type'] === 'association' && !empty($note['module_id'])) {
        ...
        <div class="col-md-6">
            <div class="card h-100">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#uploadResumeModal">Attach Resume</button>
                </div>
                <div class="card-body">
                    <?php if (!empty($candidate['resume_filename'])): ?>
                        <p><strong>Resume:</strong><br>
                            <a href="/OT2/uploads/resumes/<?= urlencode($candidate['resume_filename']) ?>" target="_blank" class="btn btn-sm btn-outline-primary mt-2">View Resume</a>
                            <a href="/OT2/uploads/resumes/<?= urlencode($candidate['resume_filename']) ?>" download class="btn btn-sm btn-outline-secondary mt-2 ms-2">Download</a>
                            <br><small class="text-muted mt-1 d-block"><?= htmlspecialchars($candidate['resume_filename']) ?></small>
                        </p>
                    <?php else: ?>
        ...
        <div class="card-body">
            <?php if ($associations): ?>
                <?php foreach ($associations as $assoc): ?>
                    ...
                    <!-- Status Modal -->
                    <div class="modal fade" id="statusModal<?= $assoc['association_id'] ?>" tabindex="-1">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <form method="POST" action="update_association_status.php">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Update Status for <?= htmlspecialchars($assoc['title']) ?></h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body">
                                        <input type="hidden" name="association_id" value="<?= $assoc['association_id'] ?>">
                                                ...
                                                <?php foreach ($statusList as $category => $statuses): ?>
                                                    <optgroup label="<?= htmlspecialchars($category) ?>">
                                                        <?php foreach ($statuses as $status): ?>
                                                            <option value="<?= htmlspecialchars($status) ?>" <?= $assoc['status'] === $status ? 'selected' : '' ?>>
