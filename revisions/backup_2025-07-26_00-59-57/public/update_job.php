...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($job_id && $title && $location && $status) {
        try {
            // Prepare and execute update (now includes client_id)
            $stmt = $pdo->prepare("UPDATE jobs SET title = ?, location = ?, status = ?, client_id = ? WHERE id = ?");
            $stmt->execute([$title, $location, $status, $client_id, $job_id]);
