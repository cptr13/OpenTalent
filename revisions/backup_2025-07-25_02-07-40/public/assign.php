...
if ($prefill_candidate_id) {
    ...
    if ($row = $stmt->fetch()) {
        $prefill_candidate_name = $row['name'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($candidate_id && $job_id) {
        try {
            ...
            $error_message = "Error assigning candidate: " . htmlspecialchars($e->getMessage());
        }
    }
    ...
    if ($contact_id && $job_id) {
        try {
            ...
            $success_message = "Contact assigned successfully.";
        } catch (PDOException $e) {
            $error_message = "Error assigning contact: " . htmlspecialchars($e->getMessage());
        }
    }

    if (empty($candidate_id) && empty($contact_id)) {
        $error_message = "Please select a candidate and/or a contact to assign.";
    }
}
?>

<div class="container my-4">
    <h2>Assign Candidate and/or Contact to Job</h2>
...
document.addEventListener('DOMContentLoaded', function () {
    setupAutocomplete('candidate_search', 'candidate_results', 'candidate_id', '/OT2/ajax/search_candidates.php');
    setupAutocomplete('job_search', 'job_results', 'job_id', '/OT2/ajax/search_jobs.php');
    setupAutocomplete('contact_search', 'contact_results', 'contact_id', '/OT2/ajax/search_contacts.php');
});
</script>
