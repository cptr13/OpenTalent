...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $pdo->prepare("INSERT INTO candidates (first_name, last_name, email, phone, city, state, zip, linkedin, resume_text, status, owner, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    $stmt->execute([
        $_POST['first_name'],
        $_POST['last_name'],
        $_POST['email'],
    ...
    <form method="POST">
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="text" name="first_name" class="form-control" value="<?= htmlspecialchars($current['first_name']) ?>" required>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label">Last Name</label>
                <input type="text" name="last_name" class="form-control" value="<?= htmlspecialchars($current['last_name']) ?>" required>
            </div>
        </div>
